...
// Anchor circulars live on BSE/NSE under per-issue notices. The reachability of
// the DRHP-archive landing page is a reasonable proxy for whether anchor PDF
// URLs are discoverable; we use one DRHP as the parse target since both are
// table-bearing PDFs with similar parsing strategy. In Phase 1 we will switch
// to actual anchor circular URLs once we map them per-issue.
...
export const probe: ProbeFn = async (ctx): Promise<ProbeResult> => {
  ...
  if (!py) {
    return {
      probe_id: 'P-18',
      source: 'Anchor circular PDF parsing — sample',
      url_or_endpoint: '(python/camelot stack)',
      ...
      status: 'YELLOW',
      recommended_action: 'Install Python + camelot in CI; skipping locally.',
  ...
  const list = await httpGet(LIST_URL, { headers: { Accept: 'text/html' } });
  if (!list.ok) {
    return {
      probe_id: 'P-18',
      source: 'Anchor circular PDF parsing — sample',
      ...
      freshness_or_update_frequency: 'Immutable',
      status: 'RED',
      recommended_action: 'Cannot reach discovery page.',
      fallback_source: 'manual',
      ran_at_utc: ctx.nowIso,
      latency_ms: list.latency_ms,
      notes: `discovery status=${list.status}`,
    };
  }
  ...
  const pdfLocal = join(ctx.outDir, '.scratch', 'p18.pdf');
  const outJson = join(ctx.outDir, '.scratch', 'p18.json');
  writeFileSync(pdfLocal, bin.buffer);
  let parseOk = false;
  let parsed: any = null;
  let pyErr = '';
  try {
    execFileSync(py, [join('scripts', 'probes', 'lib', 'pdf-parse.py'), 'anchor', pdfLocal, outJson], { stdio: 'pipe' });
    parseOk = true;
  } catch (e: any) {
  ...
  if (parsed) writeSample(ctx.samplesDir, 'sample-anchor-extracted.json', JSON.stringify(parsed, null, 2));
  const hasFields = !!parsed?.ok && (parsed?.page_1_table_rows ?? 0) >= 3;
  ...
  return {
    ...
    source: 'Anchor circular PDF parsing — sample',
    url_or_endpoint: pdfUrl,
    fetch_method: 'GET + python camelot/pdfplumber',
    headers_or_cookies_required: [],
    status_code: bin.status,
    response_type: 'PDF',
    fields_found: parsed?.first_5_rows ? ['table rows', 'cols'] : [],
    fields_missing: parsed?.first_5_rows ? [] : ['anchor table'],
    ...
    recommended_action: status === 'GREEN' || status === 'YELLOW'
      ? 'Anchor treemap viable in Phase 4–5.'
      : 'Manual entry of anchor list; revisit camelot flavour.',
    fallback_source: 'manual',
    ran_at_utc: ctx.nowIso,
    latency_ms: list.latency_ms + bin.latency_ms,
