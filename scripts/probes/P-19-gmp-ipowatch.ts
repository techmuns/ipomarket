...
const URL_ENDPOINT = 'https://ipowatch.in/ipo-grey-market-premium-latest-ipo-gmp/';
...
export const probe: ProbeFn = async (ctx): Promise<ProbeResult> => {
  ...
  if (res.ok) {
    ...
    writeSample(ctx.samplesDir, 'sample-gmp-ipowatch.html', truncate(res.body, 4000));
    ...
    blocked = true;
    notes = `Non-200 (Cloudflare expected). status=${res.status}`;
  }
  ...
  return {
    ...
    freshness_or_update_frequency: 'Daily, updates intra-day',
    status,
    recommended_action: status === 'GREEN' || status === 'YELLOW'
      ? 'Include in Phase 6 GMP averager.'
      : 'Skip; phase-6 feature flag stays off if all GMP probes fail.',
    fallback_source: 'P-20',
