...
const URL_ENDPOINT = 'https://www.bigshareonline.com/ipo_allotment.html';

export const probe: ProbeFn = async (ctx): Promise<ProbeResult> => {
  const res = await httpGet(URL_ENDPOINT, {
    headers: { Referer: 'https://www.bigshareonline.com/' },
  });
  if (res.ok) writeSample(ctx.samplesDir, 'registrar-bigshare.html', truncate(res.body, 3000));
