...
Generated: 2026-07-25T12:10:58.368Z
...
| P-02 | NSE — Upcoming IPOs | GREEN | 200 | JSON | 330 | Use as primary for Upcoming/Pipeline tab. |
| P-03 | NSE — Past/Recent IPOs | GREEN | 200 | JSON | 26 | Use as primary for Recently Listed. |
| P-04 | NSE — Live Subscription | RED | 200 | JSON | 338 | Fall back to BSE bidding (P-07). |
| P-05 | NSE Emerge — SME IPOs | YELLOW | 200 | JSON | 845 | Source reachable; 0 SME rows in current snapshot. |
| P-08 | SEBI — Public Issues Filings | GREEN | 200 | HTML | 2545 | Use as primary for Pipeline tab + DRHP master. |
| P-08b | SEBI — Processing Status | YELLOW | 200 | EMPTY | 348 | Endpoint reachable; show "no observations today" in Pipeline tab when empty. |
| P-09 | SEBI — DRHP PDF Download | GREEN | 200 | PDF | 2591 | PDF download pipeline viable; proceed to Phase 5 RHP parsing (P-17). |
| P-10 | NSE/BSE — DRHP Archive | RED | 200 | HTML | 11240 | Skip if SEBI (P-08) is GREEN. |
| P-15 | BSE — Historical OHLC (fallback) | GREEN | 200 | JSON | 10419 | NSE historical still blocked; use BSE historical as primary (official fallback). |
| P-15b | NSE — Equity Quote | RED | 403 | BLOCKED | 28 | Fall back to BSE quote. |
| P-16 | Ticker mapping (NSE list symbol field) | GREEN | 200 | JSON | 267 | Deterministic mapping; no manual override file needed for v1. |
| P-06 | BSE — Mainboard Public Issues | RED | 200 | HTML | 788 | Skip; NSE primary covers this. |
| P-06b | BSE SME — Public Issues | RED | 0 | ERROR | 10160 | Skip if NSE Emerge is GREEN. |
| P-07 | BSE — Subscription / Cumulative Bid Details | RED | 200 | HTML | 362 | Rely on NSE only (P-04). |
| P-11 | Registrar resolution (from NSE list) | RED | 200 | JSON | 287 | Fall back to per-issue page scrape or manual seed. |
| P-12 | MUFG Intime (Link Intime) — landing | GREEN | 200 | HTML | 446 | Store URL as link-out; do not scrape per-PAN. |
| P-13 | KFintech — landing | GREEN | 200 | HTML | 168 | Store URL as link-out only. |
| P-14 | Bigshare — landing | GREEN | 200 | HTML | 5179 | Store URL as link-out only. |
| P-14b | Maashitla — landing | GREEN | 200 | HTML | 765 | Store URL as link-out only. |
| P-17 | RHP PDF parsing — sample | YELLOW | 200 | EMPTY | 705 | PDF discovery selector may have drifted. |
| P-18 | Anchor circular PDF parsing — sample | YELLOW | 200 | EMPTY | 239 | PDF discovery selector may have drifted. |
| P-19 | GMP — IPOWatch | YELLOW | 200 | HTML | 2034 | Include in Phase 6 GMP averager. |
| P-20 | GMP — Chittorgarh | RED | 200 | HTML | 580 | Skip; Phase 6 stays off if all GMP probes fail. |
| P-21 | GMP — IPO Central | YELLOW | 200 | HTML | 147 | Include in Phase 6 GMP averager (lower weight given parsing difficulty). |
| P-22 | GMP — InvestorGain | YELLOW | 200 | HTML | 140 | Include in Phase 6 GMP averager. |
| P-23a | Broker IPO page — Zerodha (reference only) | GREEN | 200 | HTML | 5017 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-23b | Broker IPO page — Upstox (reference only) | GREEN | 200 | HTML | 22730 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-24 | Sector / industry classification (NSE + BSE) | RED | 403 | EMPTY | 1631 | Sector unreachable from probed endpoints; manual sector-map.json required for v1. |
| P-25 | Chittorgarh — IPO list + detail accessibility (Phase 5C) | GREEN | 200 | HTML | 110 | Run P-26 to evaluate field extraction precision against the captured HTML (detail_discovery_source=static). |
...
| P-27 | Zerodha — IPO detail refresh (Phase 5C, reference only) | GREEN | 200 | HTML | 4037 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-28 | Upstox — IPO detail refresh (Phase 5C, reference only) | GREEN | 200 | HTML | 22187 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-25b | Chittorgarh — detail-page accessibility retune (Phase 6A.1) | GREEN | 200 | HTML | 1030 | All 3 detail pages captured cleanly. Run P-26b to evaluate per-field extraction precision. |
| P-26b | Chittorgarh — detail field extraction retune (Phase 6A.1) | GREEN | - | JSON | 36 | Precision met (full=0.85 narrow=0.93). Ready for Phase 6A.2 planning approval. |
...
- Fallback: P-06
- Latency: 322 ms
- Ran at (UTC): 2026-07-25T12:09:05.429Z
...
> Active candidate symbol: XTRANET · Top-level keys: 0, 1, 2
...
[
  ...
  },
  {
    ...
    "noOfsharesBid": "33299…[truncated, total 1241 chars]
```
...
Reference&#32;&#35;18&#46;660f3417&#46;1784981375&#46;1872bd59
<P>https&#58;&#47;&#47;errors&#46;edgesuite&#46;net&#47;18&#46;660f3417&#46;1784981375&#46;1872bd59</P>
</BODY>
</HTML>
...
```
{
  ...
  "Propshop Events and Exhibitions Limited": "PROPSHOP"
}
```
...
```
{
  ...
  "Propshop Events and Exhibitions Limited": null
}
```
...
<head>
	...
	<title>IPO GMP Today, Live IPO Grey Market Premiu…[truncated, total 848280 chars]
```
...
> <tr> count: 18, bytes=330233

```
<!doctype html >
<!--[if IE 8]><html class="ie8" lang="en"> <![endif]-->
<!--[if IE 9]><html class="ie9" lang="en"> <![endif]-->
<!--[if gt IE 8]><!--><html lang="en-US" prefix="og: https://ogp.me/ns#"> <!--<![endif]--><head><title>IPO GMP 2026 - IPO Grey Market Premium, Latest GMP &amp; Kostak - IPO Central</title><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="pingback" href="https://ipocentral.in/xmlrpc.php" /> <script data-no-defer="1"…[truncated, total 330233 chars]
```
...
> final_url=https://upstox.com/ipo/vegorama-punjabi-angithi-limited-ipo/ | title=Vegorama Punjabi Angithi Limited IPO - Check IPO Date, Details, Price & Allotmen | render_mode=server-rendered | raw_len=232933 | rendered_len=272875 | challenge=false | headings=20 | tables=1 | doc_links=8 | labels=11
...
> mainboard: static status=200 bytes=147703 | sme: static status=200 bytes=146439 | detail_urls_discovered=40 | detail_discovery_source=static | detail_urls_picked=2 | detail-1: static status=200 bytes=227930 | detail-2: static status=200 bytes=248402 | challenges_detected=false
...
```
{
  ...
  "picked_detail_urls": [
    ...
    "https://www.chittorgarh.com/ipo/juniper-green-energy-ipo/2492/"
  ],
  "detail_titles": [
    "Ardee Industries IPO Date, Price, GMP, Review, Details",
    "Juniper Green Energy IPO Date, Price, GMP, Review, Details"
  ],
...
> details_extracted=2 | avg_precision=0.250 | official_pdf_links_on_allowlist=1 | official_pdf_links_off_allowlist=2 | d1: main=70358b tables=8 rows=38 | d2: main=75030b tables=9 rows=42
...
```
{
  ...
  "details": [
    ...
    {
      "index": 2,
      "source_url": "https://www.chittorgarh.com/ipo/juniper-green-energy-ipo/2492/",
      "found_count": 3,
      "precision_ratio": 0.3,
      "tables_parsed": 9,
      "table_rows_parsed": 42
    }
  ]
}
```
...
> final_url=https://upstox.com/ipo/vegorama-punjabi-angithi-limited-ipo/ | title=Vegorama Punjabi Angithi Limited IPO - Check IPO Date, Details, Price & Allotmen | render_mode=server-rendered | raw_len=232933 | rendered_len=275205 | challenge=false | headings=20 | tables=1 | doc_links=8 | labels=11
