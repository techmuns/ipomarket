...
Generated: 2026-08-12T20:05:55.136Z
...
| P-04 | NSE — Live Subscription | RED | 200 | JSON | 336 | Fall back to BSE bidding (P-07). |
| P-05 | NSE Emerge — SME IPOs | YELLOW | 200 | JSON | 1047 | Source reachable; 0 SME rows in current snapshot. |
| P-08 | SEBI — Public Issues Filings | GREEN | 200 | HTML | 6790 | Use as primary for Pipeline tab + DRHP master. |
...
| P-09 | SEBI — DRHP PDF Download | GREEN | 200 | PDF | 2700 | PDF download pipeline viable; proceed to Phase 5 RHP parsing (P-17). |
| P-10 | NSE/BSE — DRHP Archive | RED | 200 | HTML | 11264 | Skip if SEBI (P-08) is GREEN. |
| P-15 | BSE — Historical OHLC (fallback) | GREEN | 200 | JSON | 6909 | NSE historical still blocked; use BSE historical as primary (official fallback). |
| P-15b | NSE — Equity Quote | RED | 403 | BLOCKED | 34 | Fall back to BSE quote. |
| P-16 | Ticker mapping (NSE list symbol field) | GREEN | 200 | JSON | 272 | Deterministic mapping; no manual override file needed for v1. |
| P-06 | BSE — Mainboard Public Issues | RED | 200 | HTML | 507 | Skip; NSE primary covers this. |
| P-06b | BSE SME — Public Issues | RED | 0 | ERROR | 10486 | Skip if NSE Emerge is GREEN. |
| P-07 | BSE — Subscription / Cumulative Bid Details | RED | 200 | HTML | 506 | Rely on NSE only (P-04). |
...
| P-13 | KFintech — landing | GREEN | 200 | HTML | 99 | Store URL as link-out only. |
| P-14 | Bigshare — landing | GREEN | 200 | HTML | 952 | Store URL as link-out only. |
| P-14b | Maashitla — landing | GREEN | 200 | HTML | 749 | Store URL as link-out only. |
| P-17 | RHP PDF parsing — sample | YELLOW | 200 | EMPTY | 860 | PDF discovery selector may have drifted. |
| P-18 | Anchor circular PDF parsing — sample | YELLOW | 200 | EMPTY | 202 | PDF discovery selector may have drifted. |
| P-19 | GMP — IPOWatch | YELLOW | 200 | HTML | 1359 | Include in Phase 6 GMP averager. |
| P-20 | GMP — Chittorgarh | RED | 200 | HTML | 581 | Skip; Phase 6 stays off if all GMP probes fail. |
| P-21 | GMP — IPO Central | YELLOW | 200 | HTML | 125 | Include in Phase 6 GMP averager (lower weight given parsing difficulty). |
| P-22 | GMP — InvestorGain | YELLOW | 200 | HTML | 139 | Include in Phase 6 GMP averager. |
| P-23a | Broker IPO page — Zerodha (reference only) | GREEN | 200 | HTML | 4050 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
...
| P-24 | Sector / industry classification (NSE + BSE) | RED | 403 | EMPTY | 1715 | Sector unreachable from probed endpoints; manual sector-map.json required for v1. |
| P-25 | Chittorgarh — IPO list + detail accessibility (Phase 5C) | GREEN | 200 | HTML | 135 | Run P-26 to evaluate field extraction precision against the captured HTML (detail_discovery_source=static). |
...
| P-27 | Zerodha — IPO detail refresh (Phase 5C, reference only) | GREEN | 200 | HTML | 3437 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-28 | Upstox — IPO detail refresh (Phase 5C, reference only) | GREEN | 200 | HTML | 4505 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-25b | Chittorgarh — detail-page accessibility retune (Phase 6A.1) | GREEN | 200 | HTML | 981 | All 3 detail pages captured cleanly. Run P-26b to evaluate per-field extraction precision. |
...
- Fallback: P-06
- Latency: 420 ms
- Ran at (UTC): 2026-08-12T20:04:45.251Z

> Total rows: 8, Active rows: 3
...
> Active candidate symbol: BLEL · Top-level keys: 0, 1, 2, 3, 4, 5
...
[
  ...
  },
  {
    ...
    "noOfsharesBid": "…[truncated, total 2187 chars]
```
...
Reference&#32;&#35;18&#46;b7fd117&#46;1786565116&#46;7c6f2fa
<P>https&#58;&#47;&#47;errors&#46;edgesuite&#46;net&#47;18&#46;b7fd117&#46;1786565116&#46;7c6f2fa</P>
</BODY>
</HTML>
...
<head><style>img.lazy{min-height:1px}</style><link href="https://ipowatch.in/wp-content/plugins/w3-total-cache/pub/js/lazyload.min.js" as="script">
	...
	<!-- This site is optimized with the Yoast SEO Premium p…[truncated, total 887730 chars]
```
...
<head>
    ...
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="pingback" href="https://ipocentral.in/x…[truncated, total 500378 chars]
```
...
> final_url=https://upstox.com/ipo/vegorama-punjabi-angithi-limited-ipo/ | title=Vegorama Punjabi Angithi Limited IPO - Check IPO Date, Details, Price & Allotmen | render_mode=server-rendered | raw_len=235304 | rendered_len=279566 | challenge=false | headings=20 | tables=1 | doc_links=8 | labels=11
...
<H1>Access Denied</H1>
 …[truncated, total 393 chars] ; [NSE IPO list] 8 IPOs; per-row keys: companyName, issueEndDate, issuePrice, issueSize, issueStartDate, series, status, symbol; sector fields: (none) ; [BSE IPO list] BSE HTML; sector-related words found: (none); bytes=13850
...
> mainboard: static status=200 bytes=147986 | sme: static status=200 bytes=149504 | detail_urls_discovered=40 | detail_discovery_source=static | detail_urls_picked=2 | detail-1: static status=200 bytes=285232 | detail-2: static status=200 bytes=268285 | challenges_detected=false
...
> details_extracted=2 | avg_precision=0.500 | official_pdf_links_on_allowlist=0 | official_pdf_links_off_allowlist=2 | d1: main=85507b tables=11 rows=65 | d2: main=80574b tables=11 rows=66
...
> final_url=https://upstox.com/ipo/vegorama-punjabi-angithi-limited-ipo/ | title=Vegorama Punjabi Angithi Limited IPO - Check IPO Date, Details, Price & Allotmen | render_mode=server-rendered | raw_len=235304 | rendered_len=278098 | challenge=false | headings=20 | tables=1 | doc_links=8 | labels=11
