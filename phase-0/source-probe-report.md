...
Generated: 2026-08-30T21:55:50.679Z
...
| P-04 | NSE — Live Subscription | RED | 200 | JSON | 464 | Fall back to BSE bidding (P-07). |
| P-05 | NSE Emerge — SME IPOs | YELLOW | 200 | JSON | 917 | Source reachable; 0 SME rows in current snapshot. |
| P-08 | SEBI — Public Issues Filings | GREEN | 200 | HTML | 3437 | Use as primary for Pipeline tab + DRHP master. |
| P-08b | SEBI — Processing Status | YELLOW | 200 | EMPTY | 419 | Endpoint reachable; show "no observations today" in Pipeline tab when empty. |
...
| P-15 | BSE — Historical OHLC (fallback) | GREEN | 200 | JSON | 3343 | NSE historical still blocked; use BSE historical as primary (official fallback). |
| P-15b | NSE — Equity Quote | RED | 403 | BLOCKED | 183 | Fall back to BSE quote. |
| P-16 | Ticker mapping (NSE list symbol field) | GREEN | 200 | JSON | 313 | Deterministic mapping; no manual override file needed for v1. |
| P-06 | BSE — Mainboard Public Issues | RED | 200 | HTML | 814 | Skip; NSE primary covers this. |
| P-06b | BSE SME — Public Issues | RED | 0 | ERROR | 10485 | Skip if NSE Emerge is GREEN. |
| P-07 | BSE — Subscription / Cumulative Bid Details | RED | 200 | HTML | 418 | Rely on NSE only (P-04). |
| P-11 | Registrar resolution (from NSE list) | RED | 200 | JSON | 129 | Fall back to per-issue page scrape or manual seed. |
| P-12 | MUFG Intime (Link Intime) — landing | GREEN | 200 | HTML | 1034 | Store URL as link-out; do not scrape per-PAN. |
| P-13 | KFintech — landing | GREEN | 200 | HTML | 212 | Store URL as link-out only. |
| P-14 | Bigshare — landing | GREEN | 200 | HTML | 1129 | Store URL as link-out only. |
| P-14b | Maashitla — landing | GREEN | 200 | HTML | 860 | Store URL as link-out only. |
| P-17 | RHP PDF parsing — sample | YELLOW | 200 | EMPTY | 1155 | PDF discovery selector may have drifted. |
| P-18 | Anchor circular PDF parsing — sample | YELLOW | 200 | EMPTY | 236 | PDF discovery selector may have drifted. |
| P-19 | GMP — IPOWatch | YELLOW | 200 | HTML | 1505 | Include in Phase 6 GMP averager. |
| P-20 | GMP — Chittorgarh | RED | 200 | HTML | 815 | Skip; Phase 6 stays off if all GMP probes fail. |
| P-21 | GMP — IPO Central | YELLOW | 200 | HTML | 578 | Include in Phase 6 GMP averager (lower weight given parsing difficulty). |
| P-22 | GMP — InvestorGain | RED | 200 | HTML | 168 | Skip; Phase 6 stays off if all GMP probes fail. |
| P-23a | Broker IPO page — Zerodha (reference only) | GREEN | 200 | HTML | 4998 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-23b | Broker IPO page — Upstox (reference only) | GREEN | 200 | HTML | 5397 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
| P-24 | Sector / industry classification (NSE + BSE) | RED | 403 | EMPTY | 1684 | Sector unreachable from probed endpoints; manual sector-map.json required for v1. |
| P-25 | Chittorgarh — IPO list + detail accessibility (Phase 5C) | GREEN | 200 | HTML | 3818 | Run P-26 to evaluate field extraction precision against the captured HTML (detail_discovery_source=static). |
...
| P-27 | Zerodha — IPO detail refresh (Phase 5C, reference only) | GREEN | 200 | HTML | 4321 | Use as information-architecture benchmark only. Do NOT scrape for production data. |
...
| P-25b | Chittorgarh — detail-page accessibility retune (Phase 6A.1) | GREEN | 200 | HTML | 2841 | All 3 detail pages captured cleanly. Run P-26b to evaluate per-field extraction precision. |
...
- Fallback: P-06
- Latency: 407 ms
- Ran at (UTC): 2026-08-30T21:54:37.272Z
...
[
  ...
  },
  {
    ...
    "noOfsharesBid": "4380075.0",
    "srNo": n…[truncated, total 2245 chars]
```
...
<html lang="en">
<head>
<meta content="noindex, nofollow" name=…[truncated, total 22096 chars] ; [bse:RELIANCE] BSE rows=26740, fields=1
...
Reference&#32;&#35;18&#46;873b2f17&#46;1788126901&#46;13be5b8f
<P>https&#58;&#47;&#47;errors&#46;edgesuite&#46;net&#47;18&#46;873b2f17&#46;1788126901&#46;13be5b8f</P>
</BODY>
</HTML>
...
<head><style>img.lazy{min-height:1px}</style><link href="https://ipowatch.in/wp-content/plugins/w3-total-cache/pub/js/lazyload.min.js" as="script">
	...
	<!-- This site is optimized with the Yoast SEO Premium p…[truncated, total 897993 chars]
```
...
> <tr> count: 24, bytes=561898

```
<!doctype html >
<!--[if IE 8]><html class="ie8" lang="en"> <![endif]-->
<!--[if IE 9]><html class="ie9" lang="en"> <![endif]-->
<!--[if gt IE 8]><!--><html lang="en-US" prefix="og: https://ogp.me/ns#"> <!--<![endif]--><head><title>IPO GMP Today 2026 – Live Grey Market Premium &amp; Kostak Rates - IPO Central</title><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="pingback" href="https://ipocentral.in/xmlrpc.php" /> <script data-no-defer="1…[truncated, total 561898 chars]
```
...
> final_url=https://upstox.com/ipo/vegorama-punjabi-angithi-limited-ipo/ | title=Vegorama Punjabi Angithi Limited IPO - Check IPO Date, Details, Price & Allotmen | render_mode=server-rendered | raw_len=355772 | rendered_len=399171 | challenge=false | headings=20 | tables=1 | doc_links=8 | labels=11
...
> mainboard: static status=200 bytes=150250 | sme: static status=200 bytes=150023 | detail_urls_discovered=40 | detail_discovery_source=static | detail_urls_picked=2 | detail-1: static status=200 bytes=285841 | detail-2: static status=200 bytes=271622 | challenges_detected=false
...
> final_url=https://upstox.com/ipo/vegorama-punjabi-angithi-limited-ipo/ | title=Vegorama Punjabi Angithi Limited IPO - Check IPO Date, Details, Price & Allotmen | render_mode=server-rendered | raw_len=355772 | rendered_len=399066 | challenge=false | headings=20 | tables=1 | doc_links=8 | labels=11
